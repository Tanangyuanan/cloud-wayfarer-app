...
- **有朝一日**：把阿镜的途经点、文化页和沿途票据收藏起来，等真正出发时整理为用户自己的贵州旅行计划。
...
<p align="center">
  <img src="docs/images/travel-favorites-and-planner-mobile.png" width="380" alt="移动端收藏与一键旅行计划">
</p>

用户可以从阿镜走过的站、城市文化页和旅途发现中主动收藏想去的地方，再一键生成自己的旅行计划。移动端仅在用户点击规划时请求一次当前位置，用它把可定位的收藏按距离整理；授权失败时会按收藏顺序降级生成计划。当前位置不会写入收藏、长期记忆或服务端存储。

这里有一条明确的路线主权边界：**用户的路线只来自用户主动收藏的内容，不会把阿镜自己的路线自动复制成用户的行程。** 阿镜的亲历负责提供理解与建议，决定去哪里的人仍然是用户。
...
收藏不只是书签。途经点、文化页与沿途票据被统一为同一种旅行线索；用户可以选择哪些线索进入计划，阿镜再区分自己真正停留过的地方与只通过资料理解的地方，给出有事实边界的路线说明。真正出发前，计划还会提醒用户按日期、天气和开放情况重新核验。

### 7. 用户、文旅与地方经济形成同一条价值链
...
| 收藏与规划 | 浏览器 `localStorage` 统一保存途经点、文化页与票据；Geolocation API 仅在用户发起规划时调用；距离排序失败时按收藏顺序降级 |
...
- **收藏与旅行规划闭环**：桌面端与移动端共享收藏清单，可从途经点、文化页和票据生成个人路线；位置授权为可选且不持久化。
- **127 项自动化测试全部通过**。
